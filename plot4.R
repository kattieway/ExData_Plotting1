...
#making multiply plots
par(pty="s")
par(mar=c(4,4,2,2))
par(mfrow= c(2,2))
plot(datetime, filter_data$Global_active_power, type = 'l', xlab='', ylab= 'Global Active Power')

plot(datetime, filter_data$Voltage, type='l', xlab='datetime', ylab='Voltage')
...
legend('topright', inset = .05, c('Sub_metering_1', 'Sub_metering_2', 'Sub_metering_3'),
       lty=1, lwd=2.5, col=c('black', 'red', 'blue'),box.lty=0,cex=0.8)

plot(datetime, filter_data$Global_reactive_power, type='l', xlab='datetime', ylab='Global_reactive_power')
